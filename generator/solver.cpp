...
QSS_::initData()
{
  ...
  buffer << "simulator->data = QSS_Data(" << _model->states() << ","
      << _model->discretes() << "," << _model->evs() << "," << _model->inputs()
      << ","
...
QSS_::initTime()
{
  ...
  buffer << "simulator->time = QSS_Time(" << _model->states() << ","
      << _model->evs() << "," << _model->inputs() << "," << annot->initialTime()
      << ","
    ...
    int assignments)
{
  ...
  switch(intersection.type())
  {
    ...
    case IDX_EQUAL:
      if(idx.hasRange())
      {
        ...
        if(infIdx.hasRange())
        {
          ...
          buffer << "}";
          _writer->write(&buffer, alloc);
          buffer << indent << "for(j0 = " << infIdx.begin() << "; j0 <= "
              << infIdx.end() << "; j0++)";
          _writer->write(&buffer, init);
          buffer << indent << "{";
          _writer->write(&buffer, init);
          buffer << _writer->indent(2) << initStr << "[i0][" << counter
              << "[i0]++] = " << infIdx.print("j0", -infIdx.begin(), false)
              << ";";
          _writer->write(&buffer, init);
          buffer << indent << "}";
          _writer->write(&buffer, init);
        ...
        else
        {
          ...
          _writer->write(&buffer, alloc);
          buffer << indent << initStr << "[i0][" << counter << "[i0]++] = "
      ...
      else
      {
        ...
        if(infIdx.hasRange())
        {
          buffer << "for(i0 = " << infIdx.begin() << "; i0 <= " << infIdx.end()
              << "; i0++)";
          _writer->write(&buffer, init);
          buffer << "{";
          _writer->write(&buffer, init);
          buffer << indent << initStr << "[" << idxStr << "][" << counter << "["
              << idxStr << "]++] = " << infIdx.print("i", -infIdx.begin())
              << ";";
          _writer->write(&buffer, init);
          buffer << "}";
          _writer->write(&buffer, init);
        ...
        {
          buffer << initStr << "[" << idxStr << "][" << counter << "[" << idxStr
              << "]++] = " << infIdx.print("i") << ";";
          _writer->write(&buffer, init);
          if(_parallel)
          {
    ...
    case IDX_ARB_AB:
      ...
      buffer << indent << initStr << "[i0][" << counter << "[i0]++] = "
          << dIdx->print("i",
              -dIdx->offset() + infIdx.mappedBegin() + idx.low())
          << ";";
      _writer->write(&buffer, init);
      buffer << "}";
      _writer->write(&buffer, alloc, false);
      ...
      if(_parallel)
      {
        Index inf(infIdx);
        inf.setOffset(
            inf.offset() - dIdx->offset() + infIdx.mappedBegin() + idx.low());
        Index st(idx);
        st.setOffset(0);
    ...
    map<Index, Index> events)
{
      ...
      hndIdx = deps->next(type))
  {
    ...
    for(int eNum = 0; eNum < evCount; eNum++)
    {
      ...
      for(Index *zcIdx = zcDeps->begin(type); !zcDeps->end(type);
          zcIdx = zcDeps->next(type))
      {
        ...
        _indexDependencies(index, hndIdx, ecIndex, zcIdx, &simpleZCNDeps,
            WR_ALLOC_LD_HZ, WR_INIT_LD_HZ, "modelData->nHZ", "modelData->HZ",
            "events",
            is, assignments);
      }
      if(_parallel && type == DEP_DISCRETE)
      {
        ...
        for(Index *hdIdx = hndDeps->begin(type); !hndDeps->end(type); hdIdx =
            hndDeps->next(type))
        {
          ...
          _indexDependencies(index, hndIdx, ecIndex, hdIdx,
              &simpleDDDependencies, WR_ALLOC_LD_DD, WR_INIT_LD_DD,
              "modelData->nDD", "modelData->DD",
            ...
            hdIdx = hndDeps->next(DEP_DISCRETE_VECTOR))
        {
          ...
          if(is.type() != IDX_DISJOINT)
          {
            ...
            buffer << indent << "modelData->DD[" << sIdx << "][events[" << sIdx
                << "]++] = " << hhIdx << ";";
            _writer->write(&buffer, WR_INIT_LD_DD);
            _writer->write("}", WR_ALLOC_LD_DD);
            _writer->write("}", WR_INIT_LD_DD);
            _common->graphInsert(e->index(), ev->index(), _model->states(),
                NOD_DD, assignments);
          }
        }
        /* if (!(ev->index () == e->index ()))
      ...
      deps->next(eventType))
  {
    ...
    for(int eNum = 0; eNum < evCount; eNum++)
    {
      ...
      for(zcDeps->begin(type); !zcDeps->end(type); zcDeps->next(type))
      {
        ...
        _indexDependencies(index, hndIdx, ecIndex, &algVar, &simpleZCNDeps,
            WR_ALLOC_LD_HZ, WR_INIT_LD_HZ, "modelData->nHZ", "modelData->HZ",
            "events",
            is, assignments);
      }
...
void
QSS_::_eventVectorDependencies(Index index, Dependencies deps, WR_Section alloc,
...
QSS_::initializeMatrices()
{
  ...
  for(MMO_Equation e = et->begin(); !et->end(); e = et->next())
  {
    ...
    if(et->beginGenericDefinition())
    {
      ...
      if(!hasInit)
      {
        bufferGen << "for(i0 = " << index.begin() << "; i0 <= " << index.end()
            << "; i0++)";
      }
    }
    ...
    if(!deps->autonomous())
    {
      ...
      if(e->lhs().hasRange())
      {
        buffer << indent << "for (i0 = " << e->lhs().begin() << "; i0 <= "
            << e->lhs().end() << ";i0++)";
        ...
        idx = deps->next(DEP_STATE))
    {
      ...
      buffer << indent << "modelData->DS[" << eqsIdx << "][states[" << eqsIdx
          << "]++] = " << sIdx << ";";
    ...
    _common->addAlgebriacDeps(deps, e->lhs(), defStates, "modelData->nDS",
        "modelData->nSD", "modelData->DS", "modelData->SD", WR_ALLOC_LD_ALG_DS,
        WR_ALLOC_LD_ALG_SD, WR_INIT_LD_ALG_DS, WR_INIT_LD_ALG_SD, "states",
        "states", DEP_ALGEBRAIC_STATE, _modelDeps);
        ...
        dIdx = deps->next(DEP_DISCRETE))
    {
      ...
      for(MMO_Event ev = evt->begin(); !evt->end(); ev = evt->next())
      {
        ...
        for(Index *eIdx = eDeps->begin(DEP_DISCRETE); !eDeps->end(DEP_DISCRETE);
            eIdx = eDeps->next(DEP_DISCRETE))
        {
          ...
          _indexDependencies(eIndex, eIdx, equationIndex, dIdx, &simpleHDDeps,
              WR_ALLOC_LD_HD, WR_INIT_LD_HD, "modelData->nHD", "modelData->HD",
              "events", is, assignments);
      ...
      for(MMO_Event ev = evt->begin(); !evt->end(); ev = evt->next())
      {
            ...
            eIdx = eDeps->next(DEP_DISCRETE))
        {
          ...
          _indexDependencies(eIndex, eIdx, equationIndex, &algState,
              &simpleHDDeps, WR_ALLOC_LD_HD, WR_INIT_LD_HD, "modelData->nHD",
              "modelData->HD",
              "events", is, assignments);
          HD[eIndex].push_back(is);
        }
    ...
    for(Index *dIdx = deps->begin(DEP_DISCRETE_VECTOR);
        !deps->end(DEP_DISCRETE_VECTOR); dIdx = deps->next(DEP_DISCRETE_VECTOR))
    {
    ...
    _common->vectorDependencies(e->lhs(), deps, WR_ALLOC_LD_DS, WR_INIT_LD_DS,
        "modelData->nDS", "modelData->DS", WR_ALLOC_LD_SD, WR_INIT_LD_SD,
        "modelData->nSD", "modelData->SD", "states", "states", true,
  ...
  for(MMO_Event e = evt->begin(); !evt->end(); e = evt->next())
  {
    ...
    _common->addAlgebriacDeps(deps, index, defStates, "modelData->nZS",
        "modelData->nSZ", "modelData->ZS", "modelData->SZ", WR_ALLOC_LD_ALG_ZS,
        WR_ALLOC_LD_ALG_SZ, WR_INIT_LD_ALG_ZS, WR_INIT_LD_ALG_SZ, "events",
        "states");
    _common->vectorDependencies(index, deps, WR_ALLOC_LD_ZS, WR_INIT_LD_ZS,
        "modelData->nZS", "modelData->ZS", WR_ALLOC_LD_SZ, WR_INIT_LD_SZ,
        "modelData->nSZ", "modelData->SZ", "events", "states", false,
        DEP_STATE_VECTOR, &_initializeVars);
        ...
        idx = deps->next(DEP_STATE))
    {
      ...
      if(_parallel)
      {
        buffer << indent << "modelData->SH[" << idxStr << "][states[" << idxStr
            << "]++] = " << eIdx << ";";
        _writer->write(&buffer, WR_INIT_STATE_HANDLERS);
    ...
    _eventVectorDependencies(index, deps, WR_ALLOC_EVENT_DSC, WR_INIT_EVENT_DSC,
        "nLHSDsc", "LHSDsc", DEP_DISCRETE_VECTOR);
    _eventVectorDependencies(index, deps, WR_ALLOC_EVENT_LHSST,
        WR_INIT_EVENT_LHSST, "nLHSSt", "LHSSt", DEP_STATE_VECTOR);
    deps = e->deps();
    ...
    _common->addAlgebriacDeps(deps, index, defStates,
        ...
        WR_ALLOC_EVENT_ALG_RHSST, WR_NULL, WR_INIT_EVENT_ALG_RHSST, WR_NULL,
        "events", "");
      ...
      eq = equations->next())
  {
    ...
    if(idx.hasRange())
    {
      _common->genericDefCondition(idx, eq->lhs(), WR_MODEL_GENERIC,
          &_modelVars);
      _common->print(
          eq->print(indent, "dx", "j", true, algebraics, EQ_DERIVATIVE, order,
              false, 0, true, idx.low()), WR_MODEL_GENERIC);
      buffer << "}";
    ...
    else
    {
      ...
      _common->print(
          eq->print(_writer->indent(2), "dx", "", true, algebraics,
              EQ_DERIVATIVE, order, true), WR_MODEL_SIMPLE);
    ...
    bool constant)
{
    ...
    if(d->isVector(dIdx))
    {
      ...
      for(eq = eqs.begin(); eq != eqs.end(); eq++)
      {
        ...
        if(lhsEq.hasRange())
        {
          ...
          buffer << _writer->indent(indent) << "for(" << iter << "0 = "
              << lhsEq.begin() << "; " << iter << "0 <= " << lhsEq.end() << "; "
              << iter
              << "0++)";
        ...
        _common->print(
            (*eq)->print(_writer->indent(indent), aLhs.str(), iter, false,
            NULL,
      ...
      for(eq = eqs.begin(); eq != eqs.end(); eq++)
      {
        ...
        else
        {
          if(dIdx->factor() != 1 || dIdx->operConstant() != 0)
          {
            ...
            variableMod = true;
            lhs << "alg[("
                << (*eq)->lhs().print(dIdx->definition("j0"),
                    -(*eq)->lhs().operConstant(), false) << ")";
            varIdx = Util::getInstance()->getVarName(vm);
          }
          else
          {
            lhs << "alg[("
                << (*eq)->lhs().print("j0", -(*eq)->lhs().operConstant(), false)
                << ")";
        ...
        {
          _writer->write(
              (*eq)->printRange("j0", vn, _writer->indent(indent),
                  (*eq)->lhs()), s);
        }
        if(variableMod)
        {
          _writer->write(
              (*eq)->printRange(vn, vm, _writer->indent(indent), *dIdx, true),
              s);
        }
        _common->print(
      ...
      dIdx = d->next(DEP_STATE))
  {
    ...
    {
      Error::getInstance()->add(0, EM_CG | EM_NO_EQ, ER_Error,
          "Print state dependencies for index %s.", dIdx->print().c_str());
      return;
    }
    list<MMO_Equation>::iterator eq;
    for(eq = eqs.begin(); eq != eqs.end(); eq++)
    {
      ...
      _common->print(
          (*eq)->print(_writer->indent(indent), lhs.str(),
              Util::getInstance()->getVarName(varIdx), false,
              NULL,
              EQ_DEPENDENCIES, order, constantPrint, 0, true, dIdx->low(),
              constantOffset),
          s);
...
QSS_::modelDeps()
{
  ...
  for(Dependencies d = _modelDeps->begin(); !_modelDeps->end();
      d = _modelDeps->next())
  {
    ...
    else
    {
      ...
      _printDeps(d, eqIdx, equations, algebraics, "", WR_MODEL_DEPS_SIMPLE, 2,
          true);
  ...
  for(Dependencies d = _modelVectorDeps->begin(); !_modelVectorDeps->end(); d =
      _modelVectorDeps->next())
  {
        ...
        idx = d->next(DEP_STATE_VECTOR))
    {
      ...
      {
        Error::getInstance()->add(0, EM_CG | EM_NO_EQ, ER_Error,
            "Vector dependencies for index %s.", idx->print().c_str());
        return;
      }
      list<MMO_Equation>::iterator eq;
      for(eq = eqs.begin(); eq != eqs.end(); eq++)
      {
        ...
        _common->print(
            (*eq)->print(_writer->indent(1), lhs.str(), idxVar, true,
                algebraics, EQ_DEPENDENCIES, order), WR_MODEL_DEPS_GENERIC);
        _common->insertLocalVariables(&_modelDepsVars, (*eq)->getVariables());
        if(genericDef)
        {
...
Classic_::initModel()
{
  ...
  buffer << "simulator->model = CLC_Model(MOD_definition," << zeroCrossing
      << "," << handlerPos << "," << handlerNeg << ", MOD_jacobian);";
  return buffer.str();
}
...
Classic_::model()
{
      ...
      eq = algebraics->next())
  {
    ...
    if(lhs.hasRange())
    {
      ...
      _common->print(
          eq->print(_writer->indent(idt), "alg[" + lhs.print("i") + "]", "i",
              false, algebraics, EQ_CLASSIC, order), WR_MODEL_SIMPLE);
    ...
    else
    {
      _common->print(
          eq->print(indent, "alg[" + lhs.print("i") + "]", "", false,
              algebraics, EQ_CLASSIC, order, true), WR_MODEL_SIMPLE);
      _common->insertLocalVariables(&_modelVars, eq->getVariables());
    }
  }
  for(MMO_Equation eq = equations->begin(); !equations->end();
      eq = equations->next())
  {
    ...
    else
    {
      _common->print(
          eq->print(indent, "dx[" + lhs.print("i") + "]", "", false, algebraics,
              EQ_CLASSIC, order, true), WR_MODEL_SIMPLE);
      _common->insertLocalVariables(&_modelVars, eq->getVariables());
    }
  }
}
...
void
Classic_::_printDeps(Dependencies d, Index derivativeIndex,
    MMO_EquationTable equations, MMO_EquationTable algebraics, string idxStr,
    WR_Section s, int i,
    bool constant, Index infIdx)
{
      ...
      for(eq = eqs.begin(); eq != eqs.end(); eq++)
      {
        ...
        else
        {
          if(dIdx->factor() != 1 || dIdx->operConstant() != 0)
          {
            ...
            variableMod = true;
            lhs << "alg[("
                << (*eq)->lhs().print(dIdx->definition("j"),
                    -(*eq)->lhs().operConstant()) << ")";
            varIdx = vm;
          }
          else
          {
            lhs << "alg[("
                << (*eq)->lhs().print("j", -(*eq)->lhs().operConstant()) << ")";
            varIdx = vn;
          }
        }
        if(!constantPrint)
        ...
        {
          _writer->write(
              (*eq)->printRange("j", vn, _writer->indent(indent), (*eq)->lhs()),
              s);
        }
        if(variableMod)
        {
      ...
      dIdx = d->next(DEP_STATE))
  {
    ...
    for(eq = eqs.begin(); eq != eqs.end(); eq++)
    {
      ...
      {
        _common->print(
            printEq->print(_writer->indent(indent), lhs.str(), varIdx, false,
            NULL,
                EQ_JACOBIAN, order, constantPrint, 0, true, dIdx->low(),
                constant),
            s);
        _common->insertLocalVariables(&_modelDepsVars, (*eq)->getVariables());
      }
...
void
Classic_::_reorderSD(Dependencies d, const Index& idx, const string& indent,
...
Classic_::modelDeps()
{
      ...
      d = _modelDeps->next())
  {
    ...
    if(!idx.hasRange())
    {
      buffer << indent << "case " << idx.print() << ":";
      _writer->write(&buffer, WR_MODEL_DEPS_SIMPLE);
      _printDeps(d, eqIdx, equations, algebraics, "", WR_MODEL_DEPS_SIMPLE, 3,
          true, idx);
  ...
  for(i = c; i >= 0; i--)
  {
    ...
    if(idx.hasRange())
    {
      ...
      _common->genericDefCondition(eqIdx, idx, WR_MODEL_DEPS_GENERIC,
          &_modelDepsVars);
      _printDeps(d, eqIdx, equations, algebraics, idx.print("j"),
          WR_MODEL_DEPS_GENERIC, 2, false, idx);
...
Classic_::_jacobian()
{
  ...
  {
    _writer->beginBlock();
    for(map<string, string>::iterator it = _modelDepsVars.begin();
        it != _modelDepsVars.end(); it++)
    {
      _writer->print(it->second);
    }
    _writer->print("int jit = 0;");
    _writer->print("int i0 = 0;");
    buffer << "for( i0 = 0; i0 < " << _model->states() << "; i0++)";
    _writer->print(&buffer);
    _writer->print("{");
    if(!_writer->isEmpty(WR_MODEL_DEPS_SIMPLE))
    {
      _writer->print("switch (i0)");
      _writer->print("{");
